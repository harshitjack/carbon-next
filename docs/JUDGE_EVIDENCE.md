...
| â‰¥80% coverage threshold | `backend/pyproject.toml` → `fail_under = 80` |
...
|---------|-----------------|------|
| OpenRouter (Gemini Flash) | AI insight generation | `backend/app/services/gemini_service.py` |
| Supabase PostgreSQL | Carbon history storage | `backend/app/services/supabase_service.py` |
| analytics_events table | Anonymised analytics | `backend/app/services/analytics_service.py` |
| event_queue table | Async event publishing | `backend/app/services/event_queue_service.py` |
| Vercel | Frontend SPA hosting | `vercel.json`, `.github/workflows/deploy.yml` |
| GHCR / Docker | Backend container | `Dockerfile`, `.github/workflows/deploy.yml` |
