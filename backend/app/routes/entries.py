...
POST /api/entries       — Persist a carbon entry + insights to Supabase PostgreSQL.
...
async def save_entry(request: Request, body: SaveEntryRequest) -> SaveEntryResponse:
    """Save carbon entry to Supabase PostgreSQL (or in-memory fallback)."""
    settings = get_settings()
    device_id = body.carbon_result.device_id

    if settings.db_enabled and settings.SUPABASE_DB_URL:
        try:
...
async def get_entries(
    ...
    if settings.db_enabled and settings.SUPABASE_DB_URL:
        try:
            entries = await supabase_service.get_history(
                device_id=device_id,
                limit=limit,
                db_url=settings.SUPABASE_DB_URL,
            )
            ...
            entries = await supabase_service.get_history_memory(
                device_id=device_id,
                limit=limit,
            )
    else:
...
router.add_api_route(
    ...
    description=(
        ...
        "to Supabase PostgreSQL for history tracking."
    ),
)
