...
Primary engine: OpenRouter (google/gemini-flash-1.5 via OpenRouter API).
Fallback engine: Deterministic rule-based system (always available).

After generating insights, fires background tasks to:
  - Log anonymised event to PostgreSQL analytics_events table
  - Publish event to database-backed event_queue table
"""
...
from app.services import analytics_service, event_queue_service
...
async def get_insights(request: Request, body: InsightsRequest) -> InsightsResponse:
    ...
    if settings.ai_enabled:
        try:
            ...
            source = "openrouter"
            logger.info("Insights generated by OpenRouter for device %s", body.device_id[:8])
        except GeminiUnavailableError as exc:
            logger.warning("OpenRouter unavailable, falling back to rules: %s", exc)
    ...
    # Map "openrouter" back to "gemini" for the response field Literal
    # (InsightsResponse.source is typed as Literal["gemini", "rules"])
    response_source: Literal["gemini", "rules"] = (
        "gemini" if source == "openrouter" else "rules"
    )
    ...
    if settings.analytics_enabled:
        asyncio.create_task(
            analytics_service.log_event_async(
                total_kg=result.total_kg,
    ...
    if settings.event_queue_enabled:
        asyncio.create_task(
            event_queue_service.publish_insight_request(
...
router.add_api_route(
    ...
    description=(
        ...
        "Uses OpenRouter Gemini Flash (primary) with automatic fallback to rule-based engine."
    ),
)
